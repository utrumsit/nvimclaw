...
- **No plugin→skill version handshake.** Compatibility is one-way: the skill declares `requires nvimclaw: ">=0.1.3"`, and `nvim.describe` returns `protocol_version` so agents can introspect what's actually available. There is no runtime "load skill X with plugin Y" call.
